...
class GrowthController extends CommonController {
	...
	public function ok()
	{
		...
		if($state == 2){
			...
			echo "<script>alert('请勿重复点击');window.location.href = 'http://www.renmai.com/Trading/Nofinsh.html';</script>";die;
		}
		//修改订单状态
		M('upgrade')->where($a)->save($data);	
		//检测是否有其余未付款订单
			...
			if($is_one != 0){
				...
				if($money == 3888){
					...
					if($count == 1){
						...
						M('lengque')->add($info);
						echo "<meta http-equiv='Content-Type'' content='text/html; charset=utf-8'>";
						echo "<script>alert('请点击我要升级,支付200元平台维护费');window.location.href = 'http://renmai.com';</script>";die;
					}
				}
			}
			//增加正常收益
		...
		if($is_one != 0){
			...
			if($mylevel == 1 && $mymoney>=350){
				...
				echo "<meta http-equiv='Content-Type'' content='text/html; charset=utf-8'>";
				echo "<script>alert('达到升级条件,请前往升级');window.location.href = 'http://renmai.com';</script>";die;
			}
			...
			if($mylevel >= 4 && $mymoney>=70960){
				...
				session(null);
				echo "<meta http-equiv='Content-Type'' content='text/html; charset=utf-8'>";
				echo "<script>alert('您已出局,请重新登录开始下一轮游戏');window.location.href = 'http://renmai.com';</script>";die;
			}
		}
		$this->redirect('Nofinsh');
	}
	...
	public function upgrade()
	{
		...
		if($uplevel == 5){
			...
			if($ok_up>=1){
				...
				echo "<meta http-equiv='Content-Type'' content='text/html; charset=utf-8'>";
				echo "<script>alert('未达到升级条件');</script>";
			}
		}
		//查询订单
